...
async function main() {
  ...
  console.log('💪 deployment of ERC721 Create started');

  // We get the contract to deploy
  const ERC721Create = await hre.ethers.getContractFactory("ERC721Create"); // deploy first, no constructor param dependency
  const erc721Create = await ERC721Create.deploy("LisaCollectionERC721", "LCE"); //constructor param inside () here
  //await Settings.deployed();
  const erc721CreateAddress = erc721Create.address;
  console.log("💪💪 ERC721Create deployed to:", erc721CreateAddress);
