...
async function main() {
  ...
  console.log('💪 deployment of ERC20CrowdsaleFactory started');

  // We get the contract to deploy
  const ERC20CrowdsaleFactoryCreate = await hre.ethers.getContractFactory("ERC20CrowdsaleFactory"); // deploy first, no constructor param dependency
  const erc20CrowdsaleFactory = await ERC20CrowdsaleFactoryCreate.deploy(); //constructor param inside () here
  //await Settings.deployed();
  const erc20CrowdsaleFactoryAddress = erc20CrowdsaleFactory.address;
  console.log("💪💪 ERC20CrowdsaleFactory deployed to:", erc20CrowdsaleFactoryAddress);
